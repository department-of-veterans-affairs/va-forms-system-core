...
const getInputs = (container: HTMLElement) => {
  return {
    isMilitaryBaseOutsideInput: container.querySelector(
      '#homeAddressIsMilitaryBaseOutside'
    ) as HTMLElement,
    countryInput: container.querySelector('#homeAddressCountry') as HTMLElement,
    streetAddressInput: container.querySelector(
      '#homeAddressStreetAddress'
    ) as HTMLElement,
    streetAddressLine2Input: container.querySelector(
      '#homeAddressStreetAddressLine2'
    ) as HTMLElement,
    streetAddressLine3Input: container.querySelector(
      '#homeAddressStreetAddressLine3'
    ) as HTMLElement,
    ...
    postalCodeInput: container.querySelector(
...
describe('Form Builder - AddressField', () => {
  test('renders', async () => {
    const { container } = renderForm(
      <AddressField name="homeAddress" label="" />
    );
    const {
    ...
    await waitFor(() => {
      expect(isMilitaryBaseOutsideInput.getAttribute('label')).toEqual(
        'I live on a United States military base outside of the country.'
      );
      expect(countryInput.getAttribute('label')).toEqual('Country');
      expect(streetAddressInput.getAttribute('label')).toEqual(
        'Street address'
      );
      expect(streetAddressLine2Input.getAttribute('label')).toEqual(
        'Street address line 2'
      );
      expect(streetAddressLine3Input.getAttribute('label')).toEqual(
  ...
  test('renders initial value', async () => {
    ...
    await waitFor(() => {
      ...
      expect(streetAddressInput.getAttribute('value')).toEqual(
        '510 The other st'
      );
      expect(streetAddressLine2Input.getAttribute('value')).toEqual(
        'In front of bank'
      );
      expect(streetAddressLine3Input.getAttribute('value')).toEqual(
        'Behind shop'
      );
  ...
  test('renders a fieldset and legend element with a class', async () => {
    const rf = buildRenderForm(testData);
    const { container } = rf(
      <AddressField
        name="homeAddress"
  ...
  test('Country is not united states and military checkbox is true: render new city and state field', async () => {
    ...
    const { cityInput, stateInput } = getInputs(container);
