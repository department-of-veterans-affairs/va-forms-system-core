...
const AddressField = (props: AddressProps): JSX.Element => {
    ...
    return(
        <div id={`${field.name}`}>
                ...
                required>
                    {
                        ...
                        ? CountryUSA.map((country) => {
                            return <option value={`${country.value}`} key={`${country.value}`}>
                                {country.label}
                            </option>
                        })
            ...
            <p>
                U.S. millary bases are considered a domestic address and a part of the United States.
            </p>
                ...
                label="Street address line 3" />
            {
                ...
                ? <SelectField
                    ...
                    label="APO/FPO/DPO"
                    required>
                        {MilitaryCities.map((city) => <option value={`${city.value}`} key={`${city.value}`}>{city.label}</option>)}
                </SelectField>
                : <TextField
                    id={`${field.name}City`}
                    name={`${field.name}.city`}
            ...
            }
            {
                addressData.isMilitaryBaseOutside && addressData.country === 'USA'
                    ? <SelectField
                        ...
                        value={field.value} 
                        required>
                            {MilitaryStates.map((state) => <option value={`${state.value}`} key={`${state.value}`}>{state.label}</option>)}
                    </SelectField>
                : <>
                    { !addressData.isMilitaryBaseOutside && addressData.country === 'USA'
                        ? <SelectField
                            ...
                            label="State"
                            required>
                                {States.USA.map((state) => <option value={`${state.value}`} key={`${state.value}`}>{state.label}</option>)}
                        </SelectField>
                        : <TextField
                            id={`${field.name}State`}
                            name={`${field.name}.state`}
